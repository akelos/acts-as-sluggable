<?php
define('AK_AAS_PLUGIN_FILES_DIR', AK_APP_PLUGINS_DIR.DS.'acts_as_sluggable'.DS.'installer'.DS.'files');
...
class ActsAsSluggableInstaller extends AkInstaller
{
    ...
    function up_1()
    {
        $this->files = Ak::dir(AK_AAS_PLUGIN_FILES_DIR, array('recurse'=> true));
...
",$contents))>0?true:'Could not write to '.$path);
    ...
    function _copyFiles($directory_structure, $base_path = AK_AAS_PLUGIN_FILES_DIR)
    {
    ...
    {
        $dir = str_replace(AK_AAS_PLUGIN_FILES_DIR, AK_BASE_DIR,$path);
        if(!is_dir($dir)){
            mkdir($dir);
        }
    }
    ...
    {
        $destination_file = str_replace(AK_AAS_PLUGIN_FILES_DIR, AK_BASE_DIR,$path);
    ...
    function checkForCollisions(&$directory_structure, $base_path = AK_AAS_PLUGIN_FILES_DIR)
    {
        foreach ($directory_structure as $k=>$node){
            if(!empty($this->skip_all)){
                return ;
            }
            $path = str_replace(AK_AAS_PLUGIN_FILES_DIR, AK_BASE_DIR, $base_path.DS.$node);
            if(is_file($path)){
